...
- **Homepage** (`app/page.tsx` → `app/HomeClient.tsx` → `FloatingHero.tsx`) — single choreographed hero, not split into scroll-gated sections. No central object (removed on explicit request) — instead `ElectricFlow` (glowing animated SVG current lines with traveling sparks) fills that space, alongside the floating fragrance-ingredient objects + asymmetric editorial headline ("Own a scent you love?" top-left / "Find what to wear next." bottom-right) + `ModeSwitchSearch` (toggles between `SearchBox` and `FreeTextSearch` via `AnimatePresence`, both restyled to a minimal underline input rather than a bordered box), all animating in together on load — search is visible without scrolling. See experience-spec.md's "Homepage hero: floating composition" section for the full breakdown. Don't reintroduce a central hero object or the earlier two-section scroll-gated design without checking first.
- `HeroExperience.tsx`, `MarsPlanet.tsx`, `StaticHeroFallback.tsx`, `GalaxyRing.tsx`, `ScrubVideoHero.tsx`, `HeroCrosshair.tsx`, and `CentralBottle.tsx` are all deleted — the homepage went through orbital-particle-ring → Mars-planet → real-scroll-video → floating bottle photo → the current object-free electric-flow approach. If you're tempted to build another version of the homepage hero, re-read the "Homepage hero" section of experience-spec.md first and check with the user — this has pivoted four times, it is not a stable target to casually redo.
...
`"Rotate"` (perfume bottle hero, strong), `"Open"` (nav menu trigger), `"View"` (perfume card title link), `"Search"` (homepage search inputs), `"Discover"` (floating ingredient objects), `"Enter"` (homepage mode-switch buttons). `"Explore"` was the homepage `CentralBottle`'s label — that component is deleted, so it's currently unused; fine to reuse for another hero-scale element if one gets added. Keep new labels short, single-word, uppercase-styled by the cursor component automatically — don't invent a different labeling convention per component.
